...
[![Runtime](https://img.shields.io/badge/Runtime-Skills%20Compatible-blue)](#安装)
...
```text
$wenfeng-skill 请从 /path/to/novel.txt 蒸馏一个 qingdi-novelist skill，输出到 ~/.codex/skills/qingdi-novelist
```
...
文风.skill 基于开放的 Agent Skills 目录规范：根目录有 `SKILL.md`，即可被兼容 runtime 发现。
...
把仓库 clone 到对应 runtime 的 skills 目录：

| Runtime | 安装路径 |
|---|---|
| Codex | `~/.codex/skills/wenfeng-skill/` |
| Claude Code | `~/.claude/skills/wenfeng-skill/` |
| Cursor | `~/.cursor/skills/wenfeng-skill/` |
| 其他兼容 runtime | clone 到对应的 `skills/` 目录 |

Codex 示例：

```bash
mkdir -p "${CODEX_HOME:-$HOME/.codex}/skills"
git clone https://github.com/xiongtao/wenfeng-skill "${CODEX_HOME:-$HOME/.codex}/skills/wenfeng-skill"
```
...
```text
$wenfeng-skill 请从 /Users/me/novels/example.txt 蒸馏一个 example-novelist skill，输出到 ~/.codex/skills/example-novelist
```
...
```text
$wenfeng-skill 把这本书蒸馏成 river-city-novelist，输出到 ~/.codex/skills/river-city-novelist
```
...
`SKILL.md` 是必需文件。`agents/openai.yaml` 不是发现 skill 的必需文件，但它能提供更友好的 UI 展示名、短描述和默认调用提示，因此保留。
